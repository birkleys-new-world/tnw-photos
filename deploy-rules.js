// deploy-rules.js — publishes firestore.rules via the firebaserules REST API.
const fs = require("fs");
...
function req(opts, body) {
  return new Promise((res, rej) => {
    const r = https.request(opts, (rr) => { let d = ""; rr.on("data", (c) => (d += c)); rr.on("end", () => { let j = null; try { j = JSON.parse(d); } catch (_) {} res({ code: rr.statusCode, body: j || d }); }); });
    r.on("error", rej);
...
(async () => {
  ...
  // 1. create ruleset
  const rs = await req({ hostname: "firebaserules.googleapis.com", path: `/v1/projects/${PROJ}/rulesets`, method: "POST", headers: H }, JSON.stringify({ source: { files: [{ name: "firestore.rules", content: source }] } }));
  console.log("ruleset:", rs.code, JSON.stringify(rs.body).slice(0, 200));
  const name = rs.body.name;
  // 2. release it: POST a Release object (name = releases/cloud.firestore)
  const releaseBody = { name: `projects/${PROJ}/releases/cloud.firestore`, rulesetName: name };
  const rel = await req({ hostname: "firebaserules.googleapis.com", path: `/v1/projects/${PROJ}/releases`, method: "POST", headers: H }, JSON.stringify(releaseBody));
